            ...
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-2xl font-bold flex items-center">
                    @if(Auth::user()->email === 'lucas.beyer@gmx.fr')
                        ...
                        <span class="text-dev">Modifier un revenu</span>
                    @else
                        <i class="fas fa-lemon text-lemon mr-2"></i>
                        <span class="text-lemon">Modifier un revenu</span>
                    @endif
                </h1>
                ...
                <div class="grid grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="type" id="type" required
                            class="w-full bg-darker border border-gray-700 rounded px-4 py-2 text-white focus:outline-none focus:border-dev">
                            <option value="salary" {{ $income->type === 'salary' ? 'selected' : '' }}>Salaire</option>
                            <option value="aid" {{ $income->type === 'aid' ? 'selected' : '' }}>Aide</option>
                            <option value="other" {{ $income->type === 'other' ? 'selected' : '' }}>Autre</option>
                        </select>
                    </div>
